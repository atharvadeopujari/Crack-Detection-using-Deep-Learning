...
def loss_fun(y, y_pred, tip):

    y_shape = y.shape
    tip = tf.reshape(tip, shape = (y_shape[0], 2048//4, 2560//4))
    bce_loss = keras.losses.binary_crossentropy(y, y_pred)
    tip_loss = tf.multiply(bce_loss, tip)
    total_loss = bce_loss + 4*tip_loss
...
def val():
    ...
    for step, (x, y, m) in enumerate(val_ds_batched):        
        val_loss[step], val_BA[step], val_accuracy[step]= val_step(x, y, m)
...
for epoch in range(max_epochs):
    ...
    for step, (x, y, m) in enumerate(train_ds_batched):

        step_train_loss[step], step_train_BA[step], step_train_accuracy[step] = train_step(x, y, m)
...
Unet.save(filepath = "trained_model_bce+tip.h5")
